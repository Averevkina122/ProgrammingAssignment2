...
makeCacheMatrix <- function(x = matrix()) {
	inverse <- NULL  
	set <- function(y) { ##sets cache value of matrix
		x <<- y
		inverse <<- NULL
	}
	get <- function() x  ##gets cache value of matrix
	setinverse <- function(inv) inverse <<- inv  ##sets inverse of matrix 
	getinverse <- function() inverse  ##gets inverse of matrix
...
cacheSolve <- function(x, ...) {
	inverse <- x$getinverse() ##retrieves the value of inverse from cache
	if(is.null(inverse)) {
		message("inverse not found")
		value <- x$get()
		inverse <- solve(value, ...) ##calculates inverse of matrix if value not 
found in cache
